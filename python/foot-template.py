...
def analyze_gait_cycle(gait_events, frame_ms=40):
    ...
    # 防御：落地事件序列可能含 None（该周期未检测到）。选取首个两端都有效的
    # 左脚落地对作为分析周期——数据正常时即为原来的 i=1；若存在前导 None 则自动
    # 顺延，避免 None 参与比较（int > None）导致整份报告崩溃。
    i = None
    for j in range(1, len(left_on) - 1):
        if left_on[j] is not None and left_on[j + 1] is not None:
            i = j
            break
    if i is None:
        return {}, 0, 0
